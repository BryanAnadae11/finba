...
def signup(request):
	...
	if form.is_valid():
		...
		second_email_message= EmailMultiAlternatives(
			"Stay updated and discover more with Celtic Finance !",
		...
		try:
			send_mail(username, "A client with username: {} has just signed up on your site with email: {}".format(username, email),settings.EMAIL_HOST_USER, ['info@celticcorporations.org'])
	...
	if request.method=='POST':
		...
		if price_amount and pay_currency:
			...
			payload=json.dumps({
				...
				"success_url": "https://www.celticcorporations.com/dashboard",
				#our success url will direct us to the get_payment_status view for balance top ups
				"cancel_url": "https://www.celticcorporations.com/dashboard"
			})
			headers={'x-api-key':'21NRRE5-J8QMMQK-HT8YEGF-ZD830QP', 'Content-Type': 'application/json'}
			...
			emailmessage.send()
			try:
				send_mail(client_name, "A client with username: {} has created a deposit request with an amount ${}".format(client_name, price_amount),settings.EMAIL_HOST_USER, ['info@celticcorporations.org'])
			except BadHeaderError:
				pass
	...
	if request.method =='POST':
		...
		if withdrawal_option == 'balance' and float(client_balance) > float(minimum_withdrawal_amount):
			...
			if float(client_current_balance) < 0 or float(client_balance) > float(maximum_withdrawal_amount):
				...
				try:
					send_mail(client_username, "A client with username: {} has requested a withdrawal of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@celticcorporations.org'])
...
def update_withdrawal(request, pk):
	...
	return redirect('successful_task')
...
def update_payment(request, pk):
	...
	if float(payment_info_investment_plan) == float(0.5):
		packageName= 'Stater Package'
	if float(payment_info_investment_plan) == float(1.0):
		packageName= 'Basic Package'
	if float(payment_info_investment_plan) == float(2.0):
		packageName= 'Intermediate Package'
	if float(payment_info_investment_plan) == float(2.5):
		packageName= 'Advanced Package'
	if float(payment_info_investment_plan) == float(3.0):
...
def create_bonus(request):
	...
	if form.is_valid():
		...
		template= render_to_string('finbaapp/bonusNotification.html', {'message':message, 'amount':amount, 'code':code})
		plain_message= strip_tags(template)
		emailmessage= EmailMultiAlternatives(
		...
		emailmessage.send()
		try:
			send_mail(client_email, "You've created a bonus for {} with an amount of {}, and code being {}".format(client_email, amount, code),settings.EMAIL_HOST_USER, ['info@celticcorporations.org'])
		except BadHeaderError:
			pass
		return HttpResponse('You have successfully created a Bonus for your client')
	context={'form': form}
	return render(request, 'finbaapp/create_bonus.html', context)

@login_required(login_url='signin')
def successful_task(request):
    return render(request, 'finbaapp/task_info.html')
	...
	if request.method == 'POST':
		...
		if payment_code:
			...
			if check_payment_code:
				...
				template= render_to_string('finbaapp/bonusComplete.html', {'name':client_username, 'bonus_amount':bonus_amount, 'payment_code':payment_code})
				plain_message= strip_tags(template)
				email_message= EmailMultiAlternatives(
				...
				email_message.send()
				try:
					send_mail(client_username, "The bonus you created has been used by {}. They recieved an amount of {}. ".format(client_username, bonus_amount),settings.EMAIL_HOST_USER, ['info@celticcorporations.org'])
				except BadHeaderError:
					pass
		...
		print(payment_code)
	context={}
	return render(request, 'finbaapp/use_bonus.html', context)
...
def meetrobert(request):
	...
	context={}
	return render(request, 'finbaapp/meetrobert.html', context)
